# How to provision a new environment for uneeds
...
* Enter webroot path: `/home/deployer/apps/uneeds/shared/public/`

The `.pem` files created by `certbot-auto` need to be linked so Nginx can see them:

```
ln -s /etc/letsencrypt/live/example.com/fullchain.pem /etc/ssl/uneeds.crt
ln -s /etc/letsencrypt/live/example.com/privkey.pem /etc/ssl/uneeds.key
```
...
```
/opt/certbot/certbot-auto renew --webroot-path /home/deployer/apps/uneeds/shared/public/ --dry-run
```

Finally, add this to the root crontab to enable auto-renewal:

```
0 0,12 * * * /opt/certbot/certbot-auto renew --quiet --no-self-upgrade --webroot-path /home/deployer/apps/uneeds/shared/public/
```
